...
from base_adapter import SubtitleAdapterBase, SubtitleListItem, \
    SubtitleDownloadedFile, SubtitleSearchInput
...
class A4KAdapter(SubtitleAdapterBase):
    ...
    def search(self, item: SubtitleSearchInput) -> List[SubtitleListItem]:
        ...
        for item in list_items:
            ...
            lang_list = [A4KAdapter._map_language(x["data-content"]) for x in
                         list_items[0].find("div", class_="language").find_all("i")]
            language_name, language_flag = lang_list[0]

            name = content_node.text.replace("字幕下载", "")
            file_ext = name.split(".")[-1]
            results.append(SubtitleListItem(
